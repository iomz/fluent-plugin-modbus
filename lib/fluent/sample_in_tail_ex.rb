...
class TailExInput < Input
  ...
  ### Add Wildcard
  #=============================
  config_param :read_today_file, :string, :default => false
  config_param :today_format, :string, :default => "%Y%m%d"
  #=============================

  ### Add Wildcard
  #=============================
  config_param :add_filename, :string, :default => false
  config_param :filename_format, :string, :default => '.+'
  #=============================
  ...
  def configure(conf)
    ...
    #=============================
    arr_path = []
    if @read_today_file == "true"
      file_path = "#{@path}.#{Time.now.strftime(@today_format)}*"
    else
      file_path = "#{@path}"
    end
    Dir.glob(file_path).each {|f| arr_path.push(f) }
    @path = arr_path.join(',')
    #=============================
  ...
  ### add rescue
    ...
    $log.error "configure failed", :error=>ex.message
    sleep(10)
    retry
  end
  ...
  def receive_lines(lines, path)
    ...
    lines.each {|line|
      begin
        ...
        ### filename is added to record.
        #===================
        record["filename"] = path[/#{@filename_format}/] if @add_filename == "true"
        #===================
    ...
    class IOHandler
      ...
      def on_notify
        begin
          ...
          unless lines.empty?
            ### add path
            @receive_lines.call(lines, File.basename(io.path))
